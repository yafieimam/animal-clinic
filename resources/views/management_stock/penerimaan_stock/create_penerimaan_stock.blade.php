        ...
        <div class="intro-y col-span-12">
            ...
            <div class="intro-y box p-5 grid grid-cols-12 gap-4 gap-y-3">
                <div class="col-span-12 md:col-span-6 parent">
                    <label for="kode" class="form-label">Kode {{ convertSlug(Request::segment(1)) }}
                        {{ dot() }}</label>
                ...
                <div class="col-span-12 md:col-span-6 paren {{ !Auth::user()->akses('global') ? 'disabled' : '' }}t">
                    ...
                    <select name="branch_id" id="branch_id" class="form-control branch_id select2 required">
                        ...
                        @foreach (\App\Models\Branch::where('status', true)->get() as $item)
                            <option
                                {{ !Auth::user()->akses('global') ? (Auth::user()->branch_id == $item->id ? 'selected' : '') : '' }}
                                value="{{ $item->id }}">{{ $item->lokasi }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-span-12 md:col-span-6 parent">
                    <label for="supplier_id" class="form-label">Supplier {{ dot() }}</label>
                    <select name="supplier_id" id="supplier_id" class="form-control select2 required">
                        <option value="">Pilih Supplier</option>
                        @foreach (\App\Models\Supplier::where('status', true)->get() as $item)
                ...
                <div class="col-span-12 md:col-span-8 parent">
                    <label for="nomor_faktur" class="form-label">Nomor Faktur
                        {{ dot() }}</label>
                    <input id="nomor_faktur" name="nomor_faktur" type="text" class="form-control required"
                ...
                <div class="col-span-12">
                    <table class="table table-bordered">
                        ...
                        <tfoot id="hidden-when-not-empty">
                            <tr>
                                <td colspan="8" class="text-center">Belum ada item
                                    yang ditambahkan</td>
                            </tr>
                        </tfoot>
                    </table>
                ...
                <div class="text-right mt-5 col-span-12">
                    ...
                    <button type="button" class="btn btn-primary" onclick="store()"><i class="fas fa-save mr-2"></i> Simpan
                        Perubahan</button>
                </div>
            </div>
            <!-- END: Form Layout -->
                    ...
                    <div class="col-span-12 md:col-span-4 parent">
                        ...
                        <div class="input-group parent">
                            ...
                            <input id="expired_date" readonly name="expired_date" type="text"
                                class="form-control required datepicker" placeholder="yyyy-mm-dd" value=""
                                data-single-mode="true">
                        </div>
                    </div>
                    <div class="col-span-12 md:col-span-4 parent">
                        <label for="name" class="form-label">Harga Beli {{ dot() }}</label>
                        <div class="input-group">
                            <div class="input-group-text">Rp.</div>
                            <input type="text" id="harga_satuan" class="form-control mask text-right required"
                                placeholder="Harga Beli" aria-label="Harga jual obat" onkeyup="calculateTotal()">
                        </div>
                    </div>
                    <div class="col-span-12 md:col-span-4 parent">
                        <label for="qty" class="form-label">Stok {{ dot() }}</label>
                        <div class="input-group">
                            <input id="qty" type="text" class="form-control mask-not-decimal required"
        ...
        function store() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route('storePenerimaanStock') }}',
                        data: formData,
                        type: 'post',
                        processData: false,
                        contentType: false,

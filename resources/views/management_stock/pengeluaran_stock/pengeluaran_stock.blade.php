...
@section('header_filter')
    ...
    Filter Pengeluaran Stok
@endsection

@section('content_filter')
    @include(
        '../management_stock/pengeluaran_stock/filter_pengeluaran_stock'
    )
@endsection

@section('subcontent')
    {{-- <h2 class="intro-y text-lg font-medium mt-10">{{ convertSlug($global['title']) }}</h2> --}}
    <h2 class="intro-y text-lg font-medium mt-10">Pengeluaran Stok</h2>
            ...
            <div class="flex flex-wrap items-center">
                @if (Auth::user()->akses('create'))
                    <a class="btn btn-primary shadow-md mr-2" id="tambah-data"
                        href="{{ route('createPengeluaranStock') }}">Tambah Data</a>
                @endif
    ...
    <script>
        ...
        (function() {
            table = $('#table').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('datatablePengeluaranStock') }}",
                        data: {
                            _token: '{{ csrf_token() }}',
                            branch_id() {
                            ...
                            branch_tujuan_id() {
                                return $('#branch_tujuan_id_filter').val();
                            }
                        }
                    },
                    columns: [{
        ...
        function hapus(id, param) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route('deletePengeluaranStock') }}',
                        data: {
                            id: id,
                            _token: "{{ csrf_token() }}"
                        },

...
@section('subcontent')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Form Pengeluaran Stok</h2>
    </div>
            ...
            <div class="intro-y box p-5 grid grid-cols-12 gap-4 gap-y-3">
                ...
                <div class="col-span-12 md:col-span-8 parent">
                    <label for="nomor_faktur" class="form-label">Nomor Faktur
                        {{ dot() }}</label>
                    <input id="nomor_faktur" name="nomor_faktur" type="text" class="form-control"
...
@section('script')
    <script>
        ...
        function generateKode() {
            $.ajax({
                url: "{{ route('generateKodePengeluaranStock') }}",
                type: 'get',
                data: {
                    branch_id() {
                        return $('#branch_id').val();
        ...
        function calculateTotal() {
            ...
            $('#total_harga').val(accounting.formatMoney(harga_satuan * qty, {
                precision: 0
            }));
        }
        ...
        function store() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route('storePengeluaranStock') }}',
                        data: formData,
                        type: 'post',
                        processData: false,
                        contentType: false,

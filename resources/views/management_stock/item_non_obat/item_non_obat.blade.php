...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="col-span-12 md:col-span-4 ">
            <label for="satuan_non_obat_id_filter" class="form-label">Filter Satuan Non Obat</label>
            <select name="satuan_non_obat_id_filter" id="satuan_non_obat_id_filter" class="select2filter form-control">
                <option value="">Semua Satuan Non Obat</option>
                @foreach (\App\Models\SatuanNonObat::get() as $item)
        ...
        <div class="col-span-12 md:col-span-4 ">
            ...
            <select name="kategori_filter" id="kategori_filter" class="form-control select2filter">
                <option value="">Pilih Kategori</option>
                <option value="DIJUAL BEBAS">Tanpa Resep Dokter</option>
                <option value="TIDAK DIJUAL BEBAS">Dengan Resep Dokter</option>
            </select>
        </div>
        <div class="col-span-12 md:col-span-4">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    <div class="col-span-12 parent">
                        <label for="kode" class="form-label">Kode Item Non Obat {{ dot() }}</label>
                        <div class="input-group">

                            <input id="kode" name="kode" type="text" class="form-control required not-editable"
                    ...
                    <div class="col-span-12 md:col-span-6 parent">
                        <label for="name" class="form-label">Nama {{ convertSlug($global['title']) }}
                            {{ dot() }}</label>
                        <input id="name" name="name" type="text" class="form-control required"
                            placeholder="Masukan Nama {{ convertSlug($global['title']) }}">
                    </div>
                    <div class="col-span-12 md:col-span-6 parent">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" id="harga" name="harga"
                                class="form-control mask text-right required" placeholder="Harga jual non obat">
                        </div>
                    </div>
                    <div class="col-span-12 md:col-span-6 parent">
                        <label for="name" class="form-label">Kategori Penjualan {{ dot() }}</label>
                        <select name="kategori" id="kategori" class="form-control kategori select2 required">
                            <option value="">Pilih Kategori</option>
                            <option value="DIJUAL BEBAS">Tanpa Resep Dokter</option>
                            <option value="TIDAK DIJUAL BEBAS">Dengan Resep Dokter</option>
                        </select>
                    ...
                    <div class="col-span-12 md:col-span-6 parent">
                        <label for="name" class="form-label">Satuan Non Obat {{ dot() }}</label>
                        <select name="satuan_non_obat_id" id="satuan_non_obat_id"
                            class="form-control satuan_non_obat_id select2 required">
                            <option value="">Pilih Satuan Non Obat</option>
                            @foreach (\App\Models\SatuanNonObat::where('status', true)->get() as $item)
                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-span-12 md:col-span-6 parent">
                        <label for="name" class="form-label">Jenis Item {{ dot() }}</label>
                        <select name="jenis" id="jenis" class="form-control jenis select2 required">
                            <option value="">Pilih Jenis</option>
                            <option value="PAKAN">Pakan</option>
                            <option value="NON PAKAN">Non Pakan</option>
                        </select>
                    </div>
                    <div class="col-span-12 parent">
    ...
    <script>
        ...
        (function() {
            table = $('#table').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('datatableItemNonObat') }}",
                        data: {
                            _token: '{{ csrf_token() }}',
                            satuan_non_obat_id() {
                                return $('#satuan_non_obat_id_filter').val();
                            },
                            kategori() {
                                return $('#kategori_filter').val();
                            }
                        }
                    },
                    columns: [{
        ...
        function generateKode() {
            if ($('#id').val() == '' && $('#binatang_id').val() != '') {
                $.ajax({
                    url: "{{ route('generateKodeItemNonObat') }}",
                    type: 'get',
                    data: {
                        binatang_id() {

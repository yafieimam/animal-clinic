...
<div class="shadow-md fixed bottom-0 right-0 box border rounded-full h-12 px-5 flex items-center justify-center z-50 mb-10 mr-52">
    ...
    <a href="{{ route('color-scheme-switcher', ['color_scheme' => 'default']) }}" style="background: rgb(238, 39, 38)" class="block w-8 h-8 cursor-pointer rounded-full border-4 mr-1 hover:border-slate-200 {{ $color_scheme =='default' ? 'border-slate-300 dark:border-darkmode-800/80' : 'border-white dark:border-darkmode-600' }}"></a>

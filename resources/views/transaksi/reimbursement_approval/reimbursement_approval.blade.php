            ...
            <div class="modal-content">
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    ...
                    <div class="col-span-12 parent-approval">
                        ...
                        <textarea id="keterangan_approval" name="keterangan_approval" type="text" class="form-control"
                            placeholder="Masukan Keterangan"></textarea>
                    </div>
                    ...
                    <div class="col-span-12 file-container" id="fileContainerApproval">
                    </div>
                </div>
                <!-- END: Modal Body -->
                <!-- BEGIN: Modal Footer -->
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary w-20 mr-1" id="submit" onclick="dataSubmit()">Submit</button>
                    <button type="button" class="btn btn-primary w-20 mr-1" id="reject" onclick="storeReject()">Submit</button>
    ...
    <script>
        ...
        (function() {
            table = $('#table').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('datatableReimbursementApproval') }}",
                        data: {
                            _token: '{{ csrf_token() }}'
                        }
                    },
            ...
            $('#tambah-data').click(function() {
                ...
                $('.parent-approval').addClass('hidden');
                ...
                $('#keterangan_approval').removeClass('required');

                $('#fileContainer').empty();
                indexFile = 1;
                $("#fileContainerApproval").empty();
                indexFileApproval = 1;
                $("#addFileButton").click();
            ...
            $("#fileContainerApproval").on("click", ".preview-link", function(e) {
                e.preventDefault();
                const fileInput = $(this).siblings(".file-input-field");
                if(fileInput.length > 0){
                    if (fileInput[0].files.length > 0) {
        ...
        function lihat(id) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('.parent-approval').removeClass('hidden');
                    $('.parent-approval').addClass('disabled');
                    $('.not-editable').prop('readonly', true);
                    $('#modal-tambah-data .select2').trigger('change.select2');
                    ...
                    data.data.reimbursement_file_approval.forEach(function(value, index) {
                        ...
                        fileInput.append(previewLink);
                        $("#fileContainerApproval").append(fileInput);
        ...
        function store() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route('storeReimbursementApproval') }}',
                        data: formData,
                        type: 'post',
                        processData: false,
                        contentType: false,
        ...
        function dataSubmit() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.onkeydown = previousWindowKeyDown;
                    $.ajax({
                        url: '{{ route('submitReimbursementApproval') }}',
                        data: formData,
                        type: 'post',
                        processData: false,
                        contentType: false,
        ...
        function hapus(id, param) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.onkeydown = previousWindowKeyDown;
                    $.ajax({
                        url: '{{ route('deleteReimbursementApproval') }}',
                        data: {
                            id: id,
                            _token: "{{ csrf_token() }}"
                        },
        ...
        function approve(id) {
            ...
            Swal.fire({
                title: "Approve Data",
                text: "Apakah Anda Yakin Ingin Menyetujui Pengajuan Klaim Ini.",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('approveReimbursementApproval') }}",
                        data: {
                            id
                        },
                        type: 'get',
        ...
        function reject(id) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#reset-data').addClass('hidden');
                    $('#addFileButton').addClass('hidden');
                    $('#keterangan_approval').addClass('required');
        ...
        function storeReject(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('rejectReimbursementApproval') }}",
                        data: formData,
                        type: 'post',
                        processData: false,
                        contentType: false,

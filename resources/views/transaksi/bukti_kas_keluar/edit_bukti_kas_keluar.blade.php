    ...
    <form class="grid grid-cols-12 gap-6 mt-5" id="form-data">
        <div class="intro-y col-span-12">
            ...
            <div class="intro-y box p-5 grid grid-cols-12 gap-4 gap-y-3">
                ...
                <div class="col-span-12 md:col-span-6 parent">
                    <label for="tanggal_pengeluaran" class="form-label">Tanggal Input {{ dot() }}</label>
                ...
                <div class="col-span-12 parent">
                    ...
                    <select name="metode_pembayaran" id="metode_pembayaran" class="form-control select2">
                        <option {{ $data->metode_pembayaran == 'TUNAI' ? 'selected' : '' }} value="TUNAI">TUNAI</option>
                        <option {{ $data->metode_pembayaran == 'DEBET' ? 'selected' : '' }} value="DEBET">DEBET</option>
                        <option {{ $data->metode_pembayaran == 'TRANSFER' ? 'selected' : '' }} value="TRANSFER">TRANSFER
                        </option>
                    </select>
                </div>
                <div class="col-span-6 parent non-tunai hidden">
                    <label for="nama_bank" class="form-label">Nama Bank {{ dot() }}</label>
                    <input id="nama_bank" name="nama_bank" value="{{ $data->nama_bank }}" type="text"
                ...
                <div class="col-span-6 parent non-tunai hidden">
                    ...
                    <input type="text" name="nomor_kartu" value="{{ $data->nomor_kartu }}" id="nomor_kartu"
                ...
                <div class="col-span-12 parent">
                    ...
                    <textarea id="keterangan" name="keterangan" type="text" class="form-control required"
                        placeholder="Masukan Keterangan">{{ $data->description }}</textarea>
                </div>
                ...
                <div class="col-span-12">
                    <table class="table table-bordered">
                        ...
                        <tbody id="append-data">
                            @foreach ($data->JurnalDetail as $i => $d)
                                <tr>
                                    <td>
                                        {{ $d->MasterAkunTransaksi->name }}
                                        <input type="hidden" class="master_akun_transaksi_id"
                                            name="master_akun_transaksi_id[]" value="{{ $d->master_akun_transaksi_id }}">
                                        <input type="hidden" class="redaksi" name="redaksi[]"
                                            value="{{ $d->redaksi }}">
                                        <input type="hidden" class="harga" name="harga[]"
                                            value="{{ number_format($d->harga) }}">
                                        <input type="hidden" class="index" name="index[]"
                                            value="{{ $i + 1 }}">
                                        <input type="hidden" class="jurnal_id" name="jurnal_id[]"
                                            value="{{ $d->id }}">
                                    </td>
                                    <td>
                                        {{ $d->redaksi }}
                                    ...
                                    <td class="text-center">
                                        <button type="button" class="btn btn-info btn-rounded" onclick="edit(this)"><i
                                                class="fa fa-pencil" aria-hidden="true"></i></button>
                                        <button type="button" class="btn btn-danger btn-rounded"
                                            onclick="hapus(this)"><i class="fa fa-trash" aria-hidden="true"></i></button>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot id="hidden-when-not-empty" class="hidden">
                            <tr>
                                <td colspan="4" class="text-center">Belum ada item
                                    yang ditambahkan</td>
                            </tr>
                        </tfoot>
                    </table>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    ...
                    <div class="col-span-12 mb-3 parent">
                        <label>Redaksi<span style="color:red;font-weight:bold" class="important">
                                *</span></label>
                        <div class="input-group">
                            <input id="redaksi" type="text" class="form-control required text-uppercase"
                                value="" required autocomplete="redaksi" placeholder="Masukan Redaksi">
                        </div>
                    </div>
                    <div class="col-span-12 mb-3 parent">
        ...
        function store() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    overlay(true);
                    $.ajax({
                        url: '{{ route('updateBuktiKasKeluar') }}',
                        data: formData,
                        type: 'post',
                        processData: false,
                        contentType: false,

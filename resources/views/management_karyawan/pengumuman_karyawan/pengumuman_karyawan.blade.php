...
@section('subcontent')
    <h2 class="intro-y text-lg font-medium mt-10">Pemberitahuan</h2>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    <div class="col-span-12 parent">
                        <label for="name" class="form-label">Hal Pemberitahuan {{ dot() }}</label>
                        <input id="name" name="name" type="text" class="form-control required"
                            placeholder="Tulis Hal Pemberitahuan">
                        <input type="hidden" id="id" name="id">
                        {{ csrf_field() }}
                    </div>
                    <div class="col-span-12 parent">
                        <label for="description" class="form-label">Tambahkan Isi Pemberitahuan {{ dot() }}</label>
                        <textarea id="description" name="description" type="text" class="form-control required"
                            placeholder="Tulis Isi Pemberitahuan"></textarea>
                    </div>
                </div>
                <!-- END: Modal Body -->
                <!-- BEGIN: Modal Footer -->
    ...
    <script>
        ...
        (function() {
            table = $('#table').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('datatablePengumumanKaryawan') }}",
                        data: {
                            _token: '{{ csrf_token() }}'
                        }
                    },
        ...
        function edit(id) {
            $.ajax({
                url: '{{ route('editPengumumanKaryawan') }}',
                data: {
                    id
                },
                type: 'get',
        ...
        function store() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route('storePengumumanKaryawan') }}',
        ...
        function hapus(id, param) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.onkeydown = previousWindowKeyDown;
                    $.ajax({
                        url: '{{ route('deletePengumumanKaryawan') }}',
                        data: {
                            id: id,
                            _token: "{{ csrf_token() }}"
                        },
        ...
        function gantiStatus(param, id) {
            $.ajax({
                url: "{{ route('statusPengumumanKaryawan') }}",
                data: {
                    id,
                    param
                },

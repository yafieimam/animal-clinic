...
@section('subcontent')
    ...
    <h2 class="intro-y text-lg font-medium mt-10">Ruang Rawat Inap</h2>
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="col-span-12 md:col-span-4 ">
            <label for="kategori_kamar_id_filter" class="form-label">Filter Jenis Ruang Rawat Inap</label>
            <select name="kategori_kamar_id_filter" id="kategori_kamar_id_filter" class="select2filter form-control">
                <option value="">Semua Jenis Ruang Rawat Inap</option>
                @foreach (\App\Models\KategoriKamar::get() as $item)
                    <option value="{{ $item->id }}">{{ $item->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-span-12 md:col-span-4 ">
            <label for="branch_id_filter" class="form-label block">Filter Branch</label>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    <div class="col-span-12 parent">
                        <label for="name" class="form-label">Nama Ruang Rawat Inap {{ dot() }}</label>
                        <input id="name" name="name" type="text" class="form-control required"
                            placeholder="Tulis Nama Ruang Rawat Inap">
                    ...
                    <div class="col-span-12 md:col-span-6 parent">
                        <label for="kategori_kamar_id" class="form-label">Jenis Ruang Rawat Inap {{ dot() }}</label>
                        <select name="kategori_kamar_id" id="kategori_kamar_id" class="select2 form-control required">
                            <option value="">Pilih Jenis Ruang Rawat Inap</option>
                            @foreach (\App\Models\KategoriKamar::get() as $item)
                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-12 md:col-span-6 parent">
                        <label for="kapasitas" class="form-label">Kapasitas {{ dot() }}</label>
                        <input type="text" id="kapasitas" name="kapasitas" class="form-control required mask">
                    </div>
                    <div class="col-span-12 md:col-span-6 parent">
                        <label for="tarif_per_hari" class="form-label">Tarif Per Hari {{ dot() }}</label>
                        <input type="text" id="tarif_per_hari" name="tarif_per_hari"
                            class="form-control required mask">
    ...
    <script>
        ...
        (function() {
            table = $('#table').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('datatableKamarRawatInapDanBedah') }}",
                        data: {
                            _token: '{{ csrf_token() }}',
                            branch_id() {
                            ...
                            kategori_kamar_id() {
                                return $('#kategori_kamar_id_filter').val();
                            }
                        }
                    },
                    columns: [{
        ...
        function edit(id) {
            $.ajax({
                url: '{{ route('editKamarRawatInapDanBedah') }}',
                data: {
                    id
                },
                type: 'get',
        ...
        function store() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ route('storeKamarRawatInapDanBedah') }}',
        ...
        function hapus(id, param) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.onkeydown = previousWindowKeyDown;
                    $.ajax({
                        url: '{{ route('deleteKamarRawatInapDanBedah') }}',
                        data: {
                            id: id,
                            _token: "{{ csrf_token() }}"
                        },
        ...
        function gantiStatus(param, id) {
            $.ajax({
                url: "{{ route('statusKamarRawatInapDanBedah') }}",
                data: {
                    id,
                    param
                },

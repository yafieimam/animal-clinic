...
class RasController extends Controller
{
    ...
    public function index()
    {
        ...
        // Ras::whereIn('binatang_id', [1,2])->update(['hidden' => 't']);
        return view('management_klinik/ras/ras');
    }
    ...
    public function datatable(Request $req)
    {
        ...
        return Datatables::of($data)
            ->addColumn('aksi', function ($data) {
                return view('management_klinik/ras/action_button_ras', compact('data'));
            })
    ...
    public function store(Request $req)
    {
        return DB::transaction(function () use ($req) {
            ...
            if ($req->id == null or $req->id == 'null' or $req->id == '') {
                ...
                $input['id'] = $this->model->ras()->max('id') + 1;
                ...
                $validator = Validator::make(
                    ...
                    [
                        'name.unique'        => 'Nama Ras Sudah Ada',
                    ]
                );
                ...
                $this->model->ras()->find($req->id)->update($input);
    ...
    public function status(Request $req)
    {
        ...
        return DB::transaction(function () use ($req) {
            $this->model->ras()->where('id', $req->id)
                ->update([
                    'status' => $req->param
                ]);
    ...
    public function edit(Request $req)
    {
        ...
        $data = $this->model->ras()->where('id', $req->id)->first();
    ...
    public function delete(Request $req)
    {
        ...
        return DB::transaction(function () use ($req) {
            Auth::user()->akses('delete', null, true);
            try {
                $this->model->ras()->find($req->id)->delete();
            } catch (\Throwable $th) {

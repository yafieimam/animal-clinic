...
class KategoriKamarController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('management_kamar/kategori_kamar/kategori_kamar');
    }

    public function datatable(Request $req)
    {
        $data = $this->model->kategoriKamar()->all();

        return Datatables::of($data)
            ->addColumn('aksi', function ($data) {
                return view('management_kamar/kategori_kamar/action_button_kategori_kamar', compact('data'));
            })
    ...
    public function store(Request $req)
    {
        return DB::transaction(function () use ($req) {
            ...
            if ($req->id == null or $req->id == 'null' or $req->id == '') {
                $input['id'] = $this->model->kategoriKamar()->max('id') + 1;
                ...
                $validator = Validator::make(
                    $input,
                    [
                        'name'       => 'required|unique:mka_kategori_kamar',
                    ],
                    [
                ...
                $this->model->kategoriKamar()->create($input);
                ...
                $this->model->kategoriKamar()->find($req->id)->update($input);
    ...
    public function status(Request $req)
    {
        ...
        return DB::transaction(function () use ($req) {
            $this->model->kategoriKamar()->where('id', $req->id)
                ->update([
                    'status' => $req->param
                ]);
    ...
    public function edit(Request $req)
    {
        ...
        $data = $this->model->kategoriKamar()->where('id', $req->id)->first();
    ...
    public function delete(Request $req)
    {
        ...
        return DB::transaction(function () use ($req) {
            Auth::user()->akses('delete', null, true);
            try {
                $this->model->kategoriKamar()->find($req->id)->delete();
            } catch (\Throwable $th) {

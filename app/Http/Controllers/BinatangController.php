...
class BinatangController extends Controller
{
    public $model;
    ...
    public function index()
    {
        ...
        return view('management_klinik/binatang/binatang');
    }

    public function datatable(Request $req)
    {
        $data = $this->model->binatang()->all();

        // $data = $this->model->binatang()       
        ...
        return DataTables::of($data)
            ->addColumn('aksi', function ($data) {
                return view('management_klinik/binatang/action_button_binatang', compact('data'));
            })
    ...
    public function store(Request $req)
    {
        return DB::transaction(function () use ($req) {
            ...
            if ($req->id == null or $req->id == 'null' or $req->id == '') {
                $input['id'] = $this->model->binatang()->max('id') + 1;
                ...
                $validator = Validator::make(
                    $input,
                    [
                        'kode' => 'required|min:2|unique:mk_binatang',
                        'name' => 'required|min:2|unique:mk_binatang',
                    ],
                    [
                ...
                $this->model->binatang()->create($input);

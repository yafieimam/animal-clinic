...
class GroupMenuController extends Controller
{
    ...
    public function datatable(Request $req)
    {
        ...
        return Datatables::of($data)
            ...
            ->addColumn('titleMenu', function ($data) {
                return ($data->TitleMenu != null) ? $data->TitleMenu->name : "-";
            })
    ...
    public function store(Request $req)
    {
        return DB::transaction(function () use ($req) {
            ...
            if ($req->id == null or $req->id == 'null' or $req->id == '') {
                ...
                $input['id'] = $this->model->groupMenu()->max('id') + 1;
                $input['name'] = ucwords($req->name);
                $input['title_menu_id'] = $req->title_menu_id;
                $input['type'] = $req->type;
                $input['icon'] = $req->icon;
                ...
                $this->model->groupMenu()->find($req->id)->update($input);
    ...
    public function sequence(Request $req)
    {
        Auth::user()->akses('edit', null, true);
        return DB::transaction(function () use ($req) {
            $this->model->groupMenu()->where('id', $req->id)
                ->update([
    ...
    public function edit(Request $req)
    {
        ...
        $data = $this->model->groupMenu()->where('id', $req->id)->first();
    ...
    public function delete(Request $req)
    {
        ...
        return DB::transaction(function () use ($req) {
            Auth::user()->akses('delete', null, true);
            $this->model->groupMenu()->find($req->id)->delete();

...
class TypeObatController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('management_obat/type_obat/type_obat');
    }
    ...
    public function store(Request $req)
    {
        return DB::transaction(function () use ($req) {
            ...
            if ($req->id == null or $req->id == 'null' or $req->id == '') {
                $input['id'] = $this->model->typeObat()->max('id') + 1;
                ...
                $validator = Validator::make(
                    $input,
                    [
                        'kode'       => 'required|unique:mo_type_obat',
                        'name'       => 'required|unique:mo_type_obat',
                    ],
                    [
                ...
                $this->model->typeObat()->create($input);
                ...
                $this->model->typeObat()->find($req->id)->update($input);
    ...
    public function status(Request $req)
    {
        ...
        return DB::transaction(function () use ($req) {
            $this->model->typeObat()->where('id', $req->id)
                ->update([
                    'status' => $req->param
                ]);
    ...
    public function edit(Request $req)
    {
        ...
        $data = $this->model->typeObat()->where('id', $req->id)->first();
    ...
    public function delete(Request $req)
    {
        ...
        return DB::transaction(function () use ($req) {
            Auth::user()->akses('delete', null, true);
            try {
                $this->model->typeObat()->find($req->id)->delete();
            } catch (\Throwable $th) {
